            ...
            if mid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        aditmadzs.acceptGroupInvitation(op.param1)
                        ginfo = aditmadzs.getGroup(op.param1)
                        aditmadzs.sendMessage(op.param1,"M")
                    else:
                        aditmadzs.acceptGroupInvitation(op.param1)
                        ginfo = aditmadzs.getGroup(op.param1)
                        aditmadzs.sendMessage(op.param1,"M")
               ...
               if msg.contentType == 0:
                    ...
                    if text is None:
                        ...
                        if cmd == "unsend off":
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               sendMention(msg.to, sender, "┏━━[ SELFBOT-BY:CANNIBAL ]\n")
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                try:
                                    ...
                                    aditmadzs.sendMessage(msg.to,"Grup 「"+str(ginfo.name)+"」 Aman Dari JS")
                                except:
                                    pass
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               aditmadzs.sendMessage(msg.to, "{} detik".format(str(elapsed_time/3)))
                          ...
                          if wait["selfbot"] == True:
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 Setmain['CANNIBALreadPoint'][msg.to] = msg_id
                                 Setmain['CANNIBALreadMember'][msg.to] = {}
                          ...
                          if wait["selfbot"] == True:
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 del Setmain['CANNIBALreadPoint'][msg.to]
                                 del Setmain['CANNIBALreadMember'][msg.to]
                                 aditmadzs.sendMessage(msg.to, "CCTV berhasil dinoaktifkan\n\nTanggal : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\nJam [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                            
                        elif cmd == "cyduk":
                            if msg.to in Setmain['CANNIBALreadPoint']:
                                if Setmain['CANNIBALreadMember'][msg.to] != {}:
                                    aa = []
                                    for x in Setmain['CANNIBALreadMember'][msg.to]:
                                        aa.append(x)
                                    try:
                                        arrData = ""
                                        textx = "  [ Result {} member ]    \n\n  [ Read Member ]\n1. ".format(str(len(aa)))
                                        ...
                                        pass
                                    try:
                                        del Setmain['CANNIBALSreadPoint'][msg.to]
                                        del Setmain['CANNIBALreadMember'][msg.to]
                                    except:
                                        pass
                                    Setmain['CANNIBALreadPoint'][msg.to] = msg.id
                                    Setmain['CANNIBALreadMember'][msg.to] = {}
                                else:
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                                if 'MENTION' in msg.contentMetadata.keys()!=None:
                                    ...
                                    jmlh = int(Setmain["CANNIBALlimit"])
                                    if jmlh <= 1000000:
                                        for x in range(jmlh):
                                            try:
                                                aditmadzs.sendMessage1(msg)
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              ...
                              if jumlah <= 1000000:
                                  for var in range(0,jumlah):
                                      aditmadzs.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      ki.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kk.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kc.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kb.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kd.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      ke.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kf.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kg.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      kh.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      sw.sendMessage(midd, str(Setmain["CANNIBALmessage"]))
                                      
                        elif 'Mytoken' in msg.text:
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                               aditmadzs.sendMessage(msg.to,"SELFBOT-BY:CANNIBAL\n"+aditmadzs.authToken)
                               aditmadzs.sendMessage(msg.to,"SELFBOT-BY:CANNIBAL\n"+ki.authToken)
                               aditmadzs.sendMessage(msg.to,"SELFBOT-BY:CANNIBAL\n"+kk.authToken)
                               aditmadzs.sendMessage(msg.to,"SELFBOT-BY:CANNIBAL\n"+kc.authToken)
                               aditmadzs.sendMessage(msg.to,"SELFBOT-BY:CANNIBAL\n"+kb.authToken)
...
#===========KICKOUT============#
                          ...
                          if wait["selfbot"] == True:
                            ...
                            if msg._from in admin:
                                ...
                                targets.remove(mid)
                                if targets == []:
                                    aditmadzs.sendText(msg.to,"CANNIBAL KICK OUT BYE")
                                else:
                                    for target in targets:
                                      if target not in Bots:
                                        try:
